...
const Get = () => {
    ...
    const seatGetter =()=>{
        axios.get(`https://bitbox-project.herokuapp.com/book/getSeat?pk=${pk}`)
        .then(res=>{
            console.log('succeed')
            var copy = res.data;
            setShowDTO(copy)
    ...
    useEffect(()=>{
        axios.get(`https://bitbox-project.herokuapp.com/movielist/getMovieURL?title=${showDTO.movie_title}`)
                ...
                <div className={styles.reserveTable}>
                    ...
                    <div className={styles.selectComplete}>
                        ...
                        <div className={styles.buttons}>
                            ...
                            {/* 주석처리 해주세요 */}
                            <button disabled={selectedSeat.length===0? 'disable':''} onClick={paymentComplete}>결제 건너뛰기</button>
                        </div>
                    </div>
                </div>
            </div> {/* container*/}
