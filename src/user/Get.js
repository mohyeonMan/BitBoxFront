...
const Get = () => {
    ...
    const seatGetter =()=>{
        axios.get(`https://bitbox-project.herokuapp.com:8080/book/getSeat?pk=${pk}`)
        .then(res=>{
            console.log('succeed')
            var copy = res.data;
            setShowDTO(copy)
    ...
    useEffect(()=>{
        axios.get(`https://bitbox-project.herokuapp.com:8080/movielist/getMovieURL?title=${showDTO.movie_title}`)
