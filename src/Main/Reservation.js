...
const Reservation = () => {
    ...
    const getReservation = () => {
        axios.get(`http://localhost:8080/reservation/getReservation?id=${id}`)
            .then(res => {
                var array = res.data;
                var copy = [];
                array.map(item => {
    ...
    const reservationCancel = (targetReservation) => {
        window.confirm &&
        axios.get(`http://localhost:8080/book/getSeat?pk=${targetReservation.book_pk}`)
            .then(res => {
                //pk로 가져온 상영정보
                ...
                //DB에서 삭제
                axios.post('http://localhost:8080/book/addSeat', showDTO, {
                    headers: {
    ...
    // 스토어 구매현황
    useEffect(() => {
        axios.get(`http://localhost:8080/store/myStorePaymentInfo?username=${username}`)
            .then(res => {
