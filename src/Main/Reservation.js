...
const Reservation = () => {
    ...
    const getReservation = () => {
        axios.get(`https://bitbox-project.herokuapp.com/reservation/getReservation?id=${id}`)
            .then(res => {
                var array = res.data;
                var copy = [];
                array.map(item => {
    ...
    const reservationCancel = (targetReservation) => {
        window.confirm &&
        axios.get(`https://bitbox-project.herokuapp.com/book/getSeat?pk=${targetReservation.book_pk}`)
            .then(res => {
                //pk로 가져온 상영정보
                ...
                //DB에서 삭제
                axios.post('https://bitbox-project.herokuapp.com/book/addSeat', showDTO, {
                    headers: {
                        'Content-Type': 'application/json'
                    }
                }).then(res => {
                    axios.delete(`https://bitbox-project.herokuapp.com/reservation/cancelReservation?pk=${targetReservation.pk}`)
                        .then(res => {
                            alert('예매가 취소되었습니다.')
                            getReservation();
                            closeModal();
    ...
    // 스토어 구매현황
    useEffect(() => {
        axios.get(`https://bitbox-project.herokuapp.com/store/myStorePaymentInfo?username=${username}`)
            .then(res => {
