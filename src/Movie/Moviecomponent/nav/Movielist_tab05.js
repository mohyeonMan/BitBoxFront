...
const Movielist_tab05 = () => {
    ...
    useEffect(() => {
        axios.get(already_released ? 'http://localhost:8080/movielist/getMovieList_already_on_classicsociety' : 'http://localhost:8080/movielist/getMovieList_classicsociety')
    ...
    const onSearch = (e) => {
        e.preventDefault();
        axios
            .get('http://localhost:8080/movielist/Movie_search_classicsociety', {
                    params : {
                    ...
                    list.map ((item, index) => {
                        return (
                            <ol className='Movielist_seq' key={item.movie_title}>
                                <li >
                                    <div className='Movielist_title_num' hidden>{index+1}</div>
