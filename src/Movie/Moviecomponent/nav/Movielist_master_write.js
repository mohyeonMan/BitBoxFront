...
const Movielist_master_write = () => {
    ...
    const onWriteSubmit = (e) => {
        ...
        if (sw === 1) {
           ...
           // formData.append("movieData", new Blob(JSON.stringify(movieData),{type: "application/json"}))
            console.log("업로드")
            axios.post('https://bitbox-project.herokuapp.com/movielist/upload',formData, {
                    headers : {
            ...
            console.log("영화등록")
            axios.post('https://bitbox-project.herokuapp.com/movielist/write', null, {params: form})
                 .then(() => {
                    alert("영화가 등록되었습니다.");
                    navigate("/master");
                })
    ...
    const check_movie_title = () => {
        axios
            .get(`https://bitbox-project.herokuapp.com/movielist/isExistMovie_title?movie_title=${movie_title}`)
