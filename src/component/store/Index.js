...
const StoreIndex = () => {
    ...
    useEffect(()=>{
        axios.get(`https://bitbox-project.herokuapp.com/store/getIndexCombo`)
             .then(res => setCombo(res.data))
             .catch(error => console.log(error))

        axios.get(`https://bitbox-project.herokuapp.com/store/getIndexPopcorn`)
             .then(res => setPopcorn(res.data))
             .catch(error => console.log(error))

        axios.get(`https://bitbox-project.herokuapp.com/store/getIndexDrink`)
             .then(res => setDrink(res.data))
             .catch(error => console.log(error))
        
        axios.get(`https://bitbox-project.herokuapp.com/store/getIndexSnack`)
                        ...
                        <ul className={indexStyles.category_product_inner_list}>
                            {
                                ...
                                combo.map(item => {
                                    ...
                                    const goToCart = (e) => {
                                        ...
                                        axios.get('https://bitbox-project.herokuapp.com/store/isExistCart', {params: {
                                            userName : sessionStorage.getItem("userName"),
                                            store_seq : item.store_seq
                                        }})
                                            .then(res => res.data === 'exist' ? alert('장바구니에 이미 상품이 담겨있습니다.') 
                                            :
                                            axios.post('https://bitbox-project.herokuapp.com/store/insertCart', null, {params: {
                                                count : 1,
                                                img : item.img,
                                                price : item.price,
                                                store_seq : item.store_seq,
