 ...
 const WriteForm = () => {
  ...
  const onWriteSubmit = (e) => {
    ...
    if(sw === 1) {
        ...
        axios.post('https://bitbox-project.herokuapp.com/store/imgUpload', formData, {
          headers : {
        ...
        axios.post('https://bitbox-project.herokuapp.com/store/write', null, { params: form })
             .then(() => {
  ...
  const isExistSubject = () => {
    ...
    axios.get(`https://bitbox-project.herokuapp.com/store/isExistSubject?subject=${subject}`)
         .then(res => {
