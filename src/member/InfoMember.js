...
const InfoMember = ({setNum}) => {
    ...
    const checkId = () => {
        setIdDiv('');
        axios.get(`https://bitbox-project.herokuapp.com/member/duplicationChk?username=${username}`)
            .then(res => {
    ...
    const callback = (response) => {
        ...
        if (success) {
            axios.get(`https://bitbox-project.herokuapp.com/member/existName?phoneNumber=${phoneNumber}`)
                .then(res => {
                    if (res.data === 'exist') {
    ...
    const signUpActionHandler = () => {
        axios.post('https://bitbox-project.herokuapp.com/auth/signup', null, {params: form})
            .then(res => {
